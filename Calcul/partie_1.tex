...
\draw
  ...
  (O,1.5) coordinate (m) node[left] {M}
  -- (3,5) coordinate (o) node[right] {O}
  -- (3,2) coordinate (p) node[above right] {$\overrightarrow{P}$}
  pic["$\theta$", draw=orange, <->, angle eccentricity=1.2, angle radius=1cm]
  {angle=m--o--p};
  \filldraw[fill=white,line width=1pt](4,2)circle(.12cm);
      \draw[line width=.6pt] (4,2)
                            +(-135:.12cm) -- +(45:.12cm)
                            +(-45:.12cm) -- +(135:.12cm);
\end{tikzpicture}
...
\begin{tikzpicture}
\node[circle,draw,black,scale=0.2] (O) at (1,2) {};
\node[circle,draw,black,scale=0.2] (M) at (3,4) {};
\node[circle,draw,black,scale=0.2] (P) at (3,4) {};

\draw[postaction={decorate,decoration={markings,mark=at position 0.25 with {\arrow[black,line width=1.5pt]{>}}}}](O)node[below]{O}--(M)node[above]{M};
\end{tikzpicture}
