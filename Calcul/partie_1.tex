...
\begin{tikzpicture}[
  ...
  \draw[line width=.6pt]
    (2.5,0) +(-135:.12cm) -- +(45:.12cm)
            +(-45:.12cm) -- +(135:.12cm);
  \draw (2 ,0) node[black]{$\overrightarrow{\tau}$};
  \draw

    (4,0) coordinate (m) node[left] {O}
    -- (6,3) coordinate (o) node[right] {M}
    -- (6,1) coordinate (p) node[right]{$\overrightarrow{P}$}

    pic["$\theta$", draw=orange, <->, angle eccentricity=1.2, angle radius=1cm]
   {angle=m--o--p};
   \draw (4,0) node[black]{$\bullet$};
   \draw (6,3) node[black]{$\bullet$};


   \draw[thick,->]
   (6,3) coordinate (O)  node[right]{}
   -- (6,1) coordinate (p) node[right]{};
...
\begin{tikzpicture}[
  ...
  \draw
    ...
    -- (3,1) coordinate (p) node[right]{$\overrightarrow{P}$}

    pic["$\theta$", draw=orange, <->, angle eccentricity=1.2, angle radius=1cm]
   {angle=p--o--m};
   \draw (5,0) node[black]{$\bullet$};
   \draw (3,3) node[black]{$\bullet$};

   \draw[thick,->]
   (3,3) coordinate (O)  node[right]{}
   -- (3,1) coordinate (p) node[right]{$\overrightarrow{P}$};
